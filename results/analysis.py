...
mean_df = pd.concat([bfs_8p, idfs_8p, gbfs_8p, astar_8p, idastar_8p]).groupby('Algorithm').mean()
print(mean_df.to_latex(longtable=True, caption="Average results for the 8-Puzzle instances"))

# %%
astar_15p = pd.read_csv('astar_15puzzle.csv', header=None, names=columns)
astar_lc_15p = pd.read_csv('astar_lc_15puzzle.csv', header=None, names=columns)
print(astar_15p.to_latex(longtable=True, caption="A* results for the 15-Puzzle"))
print(astar_lc_15p.to_latex(longtable=True, caption="A* with linear conflicts heuristic results for the 15-Puzzle"))
